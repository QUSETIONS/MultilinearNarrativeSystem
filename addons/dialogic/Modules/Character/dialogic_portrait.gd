...
func apply_texture(node:Node, texture_path:String) -> void:
	...
	# LEGACY CHECK REMOVED to allow force loading from file below
	...
	var img_path = ProjectSettings.globalize_path(texture_path)
	
	print("[PortraitDebug] Loading: ", texture_path)
	print("[PortraitDebug] Disk Path: ", img_path)
	print("[PortraitDebug] File Exists: ", FileAccess.file_exists(img_path))
	
	if FileAccess.file_exists(img_path) or FileAccess.file_exists(texture_path):
		var img = Image.load_from_file(img_path)
		print("[PortraitDebug] Image Load Success: ", img != null)
		if img:
	...
	print("[PortraitDebug] Final Texture Assigned: ", tex != null)
	node.texture = tex
	
	# Force visibility properties to rule out animation bugs
	node.show()
	node.modulate = Color(1, 1, 1, 1)
	
	# Debug Visibility
	print("[PortraitDebug] Node Visible: ", node.visible, " Modulate: ", node.modulate)
	# Debug Hierarchy and Position
	var curr = node
	var depth = 0
	print("[PortraitDebug] --- Hierarchy Trace ---")
	while curr and depth < 6:
		var info = "[PortraitDebug] L%d: %s (%s) Vis:%s" % [depth, curr.name, curr.get_class(), curr.visible]
		if curr is Node2D or curr is Control:
			info += " GlobalPos:%s Mod:%s" % [curr.get_global_position(), curr.modulate]
			if "z_index" in curr:
				info += " Z:%d" % curr.z_index
		print(info)
