...
@onready var mute_label: Label = $CenterWrap/HeroPanel/HeroMargin/HeroVBox/AudioRow/MuteLabel
...
func _apply_theme() -> void:
	...
	mute_label.add_theme_color_override("font_color", TEXT_SUB)
	for btn in _buttons:
		_style_button(btn)
...
func _init_audio_controls() -> void:
	...
	_is_muted = AudioServer.is_bus_mute(_audio_bus_idx)

	var db_value := AudioServer.get_bus_volume_db(_audio_bus_idx)
	if is_inf(db_value) or is_nan(db_value):
		_last_volume_linear = DEFAULT_VOLUME_LINEAR
	else:
		_last_volume_linear = clampf(db_to_linear(db_value), 0.0, 1.0)
